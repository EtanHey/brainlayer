...
def load_instagram_messages(export_path: Path) -> Iterator[UnifiedMessage]:
    ...
    for comment in comments:
        ...
        text = (
            comment.get("string_list_data", [{}])[0].get("value", "")
            if "string_list_data" in comment
            else comment.get("text", "")
        )
        timestamp_val = (
            comment.get("string_list_data", [{}])[0].get("timestamp", 0)
            if "string_list_data" in comment
            else comment.get("timestamp", 0)
        )
    ...
    if export_path.suffix.lower() == ".zip":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not activity_path:
                raise FileNotFoundError(f"Gemini Apps data not found in zip: {export_path}")
...
class UnifiedTimeline:
    ...
    def apply_chat_tags(self, tags_config_path: Optional[Path] = None) -> int:
        ...
        """
        from .chat_tags import get_tag_for_message, load_chat_tags
    ...
    def load(cls, path: Path) -> "UnifiedTimeline":
        ...
        timeline.messages = [UnifiedMessage.from_dict(m) for m in data.get("messages", [])]
        return timeline
