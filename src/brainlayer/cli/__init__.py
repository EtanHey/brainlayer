...
def _report_ingest_denylist_overrides(resolved_env_file: Path) -> None:
    """Say, loudly, what an ingest denylist override is doing -- from the FILE, not just environ.

    The blanket this exists to catch lives in the env file launchd loads, which is not in a fresh
    shell's environment, so auditing os.environ alone would stay silent on the actual defect.
    """
...
def setup(
    ...
    _report_ingest_denylist_overrides(resolved_env_file)
