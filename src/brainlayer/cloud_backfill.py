...
def export_unenriched_chunks(
    ...
    """Export pending preview-summary candidates to JSONL files (one per batch job).
    ...
    total = cursor.execute(f"SELECT COUNT(*) FROM chunks WHERE {where}", params).fetchone()[0]
    print(f"Total pending preview candidates: {total}")
        ...
        for job in pending:
            ...
            if status["state"] == "succeeded":
                ...
                save_checkpoint(
                    ...
                    batch_id=job["batch_id"],
                    status="failed",
                    error=status.get("error") or status["state"],
                    completed_at=datetime.now(timezone.utc).isoformat(),
                )
