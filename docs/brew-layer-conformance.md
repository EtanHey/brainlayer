...
> §1 and §7 rows re-verified against the code at **1.5.9** (2026-08-20).
...
- **Agents (stdio MCP):** BrainBar on `/tmp/brainbar.sock` via the packaged
  `brainlayer-mcp-stdio-bridge` (preferred) or
...
| 1 | account-rename self-heal (no interactive sudo) | ⚠️ **partial** | Cask `preflight` self-heals dangling `localaiengine`-tainted Caskroom symlinks (account-agnostic via `start_with?`, user-owned only, no sudo) — **M1-verified**. `brainlayer doctor` now exists as a **read-only** health gate (`--db`, `--queue-dir`, `--watcher-health-path`, `--drain-health-path`, `--json`, `--no-mcp-config-check`). **Gap:** it has no `--fix`, so the §1 MUST form is still unmet. |
...
| 7 | read-only commands stay read-only | ⚠️ **review** | Action-verbs mutate appropriately (`reconcile-launchd`, `watch`, `wal-checkpoint`, `drain`, `repair-fts`); `health-check`, `status`, and `doctor` are read-only. (`serve` no longer exists — the FastAPI daemon was removed.) **Open:** `setup` ("create brainlayer.env…") and `init` (interactive wizard) mutate — but both are the **documented install entrypoints** (formula caveat: "Run setup after install"), not surprise-mutating inspect-verbs. Needs standard-owner ruling on whether documented-`setup`-as-install must be renamed `install`. |

## Remaining work

1. **§1 — `brainlayer doctor --fix` (the one real MUST gap).**
   `brainlayer doctor` ships as a read-only health gate, and the *friction* is functionally fixed
   and M1-verified (cask preflight heals the reinstall case). What is still missing is the healing
   half of the standard's non-interactive CLI:
   ...
   would be lifted into `doctor --fix`. **Scoped follow-on; the M1 already works without it.**
