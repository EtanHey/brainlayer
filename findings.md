...
- 2026-05-21: Review iteration 1: Codex and Macroscope both flagged `runLaunchctlPrint` waiting before draining the `Pipe`; fixed by reading pipe data before `waitUntilExit()`. Focused DashboardTests still pass (5 tests, 0 failures), and full `swift test` still passes (377 tests, 0 failures). Macroscope's lane-unavailable note is intentional per dispatch: DB-derived lanes must render without daemon snapshot; daemon health remains reported separately.
- 2026-05-21: Review iteration 2 after CI green: Codex flagged stale PID-file reuse. Added `daemonPIDFromFile` validation so pidfile PIDs must resolve to `BrainBarDaemon` by bundle/name/executable path before being trusted, plus a regression test that rejects the current non-daemon xctest PID. Focused DashboardTests pass (6 tests, 0 failures), and full `swift test` passes (378 tests, 0 failures). Cursor Bugbot's remaining low-severity lane-unavailable note matches the intentional daemon/lane decoupling from the dispatch.
