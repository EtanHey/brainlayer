...
@pytest.mark.parametrize(
    ("payload", "expected_state", "expected_issue"),
    [
        (
            {
                "attempted_at": "2026-09-09T02:00:00+00:00",
                "status": "uploaded",
                "uploaded": True,
                "verified": True,
                "archive": "claude-jsonl-2026-09-09.tar.gz",
            },
            "verified_bundle",
            None,
        ),
        ...
        (
            {
                ...
                "uploaded": False,
                "verified": True,
                "message": "no-op, 10 files already covered",
            },
            "no_op",
            None,
        ),
...
def test_jsonl_backup_detector_distinguishes_verified_noop_and_failure(
    tmp_path, payload, expected_state, expected_issue
):
    ...
    assert (issue.code if issue else None) == expected_issue


def test_jsonl_backup_detector_reports_absent_malformed_and_stale_attempts(tmp_path):
    log_path = tmp_path / "jsonl-backup.log"
    now = datetime(2026, 9, 9, 12, 0, tzinfo=UTC)

    missing, missing_issue = health_check.inspect_jsonl_backup_health(
        log_path,
        now=now,
        max_age_seconds=36 * 60 * 60,
    )
    assert missing.state == "missing"
    assert missing_issue.code == "jsonl_backup_attempt_missing"

    log_path.write_text("not-json\n", encoding="utf-8")
    malformed, malformed_issue = health_check.inspect_jsonl_backup_health(
        log_path,
        now=now,
        max_age_seconds=36 * 60 * 60,
    )
    assert malformed.state == "invalid"
    assert malformed_issue.code == "jsonl_backup_attempt_invalid"

    log_path.write_text(
        json.dumps(
            {
                "attempted_at": "2026-09-07T02:00:00+00:00",
                "status": "uploaded",
                "uploaded": True,
                "verified": True,
                "archive": "claude-jsonl-2026-09-07.tar.gz",
            }
        )
        + "\n",
        encoding="utf-8",
    )
    stale, stale_issue = health_check.inspect_jsonl_backup_health(
        log_path,
        now=now,
        max_age_seconds=36 * 60 * 60,
...
def test_run_health_check_surfaces_jsonl_backup_failure(tmp_path):
    ...
    result = run_health_check(
        HealthCheckConfig(
            ...
            jsonl_backup_log_path=log_path,
            source_jsonl_globs=[],
            queue_dir=tmp_path / "queue",
            pending_stores_path=tmp_path / "pending-stores.jsonl",
            watcher_health_path=tmp_path / "watcher-health.json",
        ...
        socket_request_fn=_ok_canary,
        command_runner=lambda _args: SimpleNamespace(returncode=0, stdout="", stderr=""),
        now_fn=lambda: datetime(2026, 9, 9, 12, 0, tzinfo=UTC),
    )

    assert result.jsonl_backup is not None
    assert result.jsonl_backup.state == "failed"
...
def test_disabled_enrichment_unloaded_is_informational(tmp_path, monkeypatch):
    db_path = tmp_path / "brainlayer.db"
    _make_db(db_path, total=1, vector_rows=1)
...
def _queue_backlog_config(tmp_path, *, heal: bool) -> tuple[HealthCheckConfig, Path, Path, Path]:
    db_path = tmp_path / "brainlayer.db"
    state_path = tmp_path / "health-state.json"
    queue_dir = tmp_path / "queue"
    pause_path = tmp_path / "pause.sentinel"
    ...
    return config, state_path, queue_dir, pause_path


def _loaded_launchd_runner(commands: list[list[str]]) -> Callable[[list[str]], SimpleNamespace]:
...
def _run_queue_backlog_health(config, command_runner):
    return run_health_check(
        config,
        ps_output_fn=_ps_output_stub(
            "123 /usr/bin/python scripts/hotlane_brainbar_daemon.py --interval 1 --backlog-batch 4\n"
        ),
        socket_request_fn=_ok_canary,
        command_runner=command_runner,
        now_fn=lambda: datetime(2026, 9, 8, 11, 30, tzinfo=UTC),
    )
...
def test_pause_aware_heal_decision_blocks_only_an_entirely_paused_enrichment_queue(tmp_path):
    queue_dir = tmp_path / "queue"
    queue_dir.mkdir()
    queue_file = queue_dir / "enrichment-one.jsonl"
    queue_file.write_text(ENRICHMENT_EVENT, encoding="utf-8")
    pause_payload = {
        "paused_at": "2026-08-04T13:50:36Z",
        "labels": ["com.brainlayer.enrichment"],
    }
    ...
    queue_file.write_text(ENRICHMENT_EVENT, encoding="utf-8")
    pause_payload["labels"] = ["custom.enrichment"]
    assert explanation() is None

    summary = health_check._queue_heal_summary(
        config=HealthCheckConfig(),
        result=health_check.HealthCheckResult(checked_at="now", ok=False),
        blocking_reason="superseded by lock_holder_wedge",
        previous_failures={"com.brainlayer.drain:queue_backed_up": 85},
        heal_failures={},
        heal_tripped=set(),
        backlog_issue_active=True,
    )
    assert summary == "heal=skipped heal_failures=85 reason=superseded by lock_holder_wedge"


def test_paused_enrichment_backlog_reports_skipped_heal_and_prior_failure_count(tmp_path, monkeypatch):
    config, state_path, queue_dir, pause_path = _queue_backlog_config(tmp_path, heal=True)
    (queue_dir / "enrichment-paused.jsonl").write_text(ENRICHMENT_EVENT, encoding="utf-8")
    pause_path.write_text(
...
def test_unpaused_backlog_still_attempts_drain_heal_and_reports_outcome_without_toast(tmp_path, monkeypatch):
    config, _state_path, queue_dir, _pause_path = _queue_backlog_config(tmp_path, heal=True)
    ...
    assert "heal_failures=1" in queue_issue.message
    assert notifications == []
...
def test_pause_explained_backlog_stays_quiet_until_pause_lifts(tmp_path, monkeypatch):
    ...
    def run():
        return _run_queue_backlog_health(config, lambda _command: SimpleNamespace(returncode=0, stdout="", stderr=""))
...
def test_enrichment_pause_does_not_hide_unexplained_backlog_issue_but_toast_stays_silent(
    tmp_path,
    monkeypatch,
    queue_kinds,
):
    ...
    result = _run_queue_backlog_health(
        config,
        lambda _command: SimpleNamespace(returncode=0, stdout="", stderr=""),
    )

    assert "queue_backed_up" in [issue.code for issue in result.issues]
    assert notifications == []


def test_unchanged_unexplained_backlog_stays_log_only(tmp_path, monkeypatch):
...
def test_disabled_enrichment_backlog_remains_log_only_after_reenable(tmp_path, monkeypatch):
...
def test_old_paused_queue_below_auto_heal_count_does_not_page(tmp_path, monkeypatch):
    ...
    pause_path.write_text('{"labels":["com.brainlayer.enrichment"]}', encoding="utf-8")
    notifications = _capture_queue_notifications(monkeypatch)
